...
export async function generateBroadcastParams(
  ...
  const provingKey = await fetch(
    isPlonk ? `${circuitDir}/${circuitName}_plonk.zkey` : `${circuitDir}/${circuitName}.zkey`
  );
...
export async function generatePlonkBroadcastParams(
  ...
  const callData = await snarkjs.plonk.exportSolidityCallData(proof, editedPublicSignals);

  // VERY HACKY UPDATE LIB
  return [callData.split(",")[0], JSON.parse(callData.slice(callData.indexOf(",") + 1))];
}
